...
async function pickDirectoryItem(fieldKey: string) { try { const r = await postFileDialog({ mode: 'open_folder', title: '选择目录' }); if (r.status === 'selected' && r.paths.length) addDirectoryItem(fieldKey, r.paths[0]) } catch (e: any) { if (e?.status === 503) toast.info('', workspace.isLimitedBrowser ? '受限浏览器模式：系统目录选择器不可用' : '当前运行环境不支持系统目录选择器') } }
async function pickPythonPath() { try { const r = await postFileDialog({ mode: 'open_file', title: '选择 Python 可执行文件' }); if (r.status === 'selected' && r.paths.length) setField('python_runtime_settings', 'python_executable_path', r.paths[0]) } catch (e: any) { if (e?.status === 503) toast.info('', workspace.isLimitedBrowser ? '受限浏览器模式：系统文件选择器不可用' : '当前运行环境不支持系统文件选择器') } }
