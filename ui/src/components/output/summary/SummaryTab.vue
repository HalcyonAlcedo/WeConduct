...
const outcomeLabel = computed(() => {
  ...
  if (status === 'succeeded') return { text: t('framework.summary.outcome.succeeded', '编译成功'), cls: 'badge-success' }
  if (status === 'failed') return { text: t('framework.summary.outcome.failed', '编译失败'), cls: 'badge-error' }
  if (status === 'unsupported') return { text: t('framework.summary.outcome.unsupported', '不支持'), cls: 'badge-warning' }
  return null
})

function stageStatusLabel(status: string): { text: string; cls: string } {
  switch (status) {
    case 'succeeded': return { text: t('framework.summary.stageStatus.succeeded', '✓ 成功'), cls: 'ss-success' }
    case 'failed': return { text: t('framework.summary.stageStatus.failed', '✕ 失败'), cls: 'ss-error' }
    case 'pending': return { text: t('framework.summary.stageStatus.pending', '◉ 待定'), cls: 'ss-pending' }
...
<template>
  <div class="summary-tab">
    ...
    <PlaceholderBanner
      ...
      type="empty"
      :title="t('framework.summary.empty.title', '编译源代码后在此查看结果')"
      :description="t('framework.summary.empty.description', '输入源代码并点击「编译」按钮开始')"
    />
    ...
    <template v-else-if="hasResult && view">
      ...
      <div class="st-section">
        ...
        <div class="st-meta" v-if="outcome">
          {{ t('framework.summary.meta.compilationId', '编译 ID') }}: {{ outcome.compilation_summary.compilation_id }}
          <span class="st-meta-div">|</span>
          {{ t('framework.summary.meta.duration', '耗时') }}: {{ durationLabel }}
        </div>
      </div>

      <!-- Stage Breakdown -->
      <div class="st-section">
        <h3 class="st-heading">{{ t('framework.summary.stageBreakdown.title', '阶段明细') }}</h3>
        <table class="st-table">
          <thead>
            <tr>
              <th>{{ t('framework.summary.stageBreakdown.stage', '阶段') }}</th>
              <th>{{ t('framework.summary.stageBreakdown.status', '状态') }}</th>
              <th>{{ t('framework.summary.stageBreakdown.diagnosticCount', '诊断数') }}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div class="st-section" v-if="stageOverview">
        <h3 class="st-heading">{{ t('framework.summary.overview.title', '概览') }}</h3>
        <div class="st-overview-grid">
          <div class="st-ov-item">
            <span class="st-ov-label">{{ t('framework.summary.overview.totalStages', '总阶段数') }}</span>
            <span class="st-ov-value">{{ stageOverview.total_stage_count }}</span>
          </div>
          <div class="st-ov-item">
            <span class="st-ov-label">{{ t('framework.summary.overview.succeeded', '成功') }}</span>
            <span class="st-ov-value success">{{ stageOverview.succeeded_stage_count }}</span>
          </div>
          <div class="st-ov-item">
            <span class="st-ov-label">{{ t('framework.summary.overview.failed', '失败') }}</span>
            <span class="st-ov-value error">{{ stageOverview.failed_stage_count }}</span>
          </div>
          <div class="st-ov-item" v-if="stageOverview.terminal_stage">
            <span class="st-ov-label">{{ t('framework.summary.overview.terminalStage', '终止阶段') }}</span>
      ...
      <div class="st-section" v-if="diagSummary">
        <h3 class="st-heading">{{ t('framework.summary.diagnostics.title', '诊断汇总') }}</h3>
        <div class="st-ds">
          {{ t('framework.summary.diagnostics.total', '总计 {n} 条诊断', { n: diagSummary.total_count }) }}
          <span v-if="diagSummary.highest_severity">
            · {{ t('framework.summary.diagnostics.highestSeverity', '最高严重度') }}: {{ diagSummary.highest_severity }}
          </span>
        </div>
      </div>
      ...
      <div class="st-section" v-if="graphStats">
        <h3 class="st-heading">{{ t('framework.summary.graph.title', '图模型') }}</h3>
        <div class="st-ds" v-if="graphStats.graph_model_id">
          {{ graphStats.graph_model_id }}
          · {{ t('framework.summary.graph.nodes', '节点') }} {{ graphStats.node_count }}
          · {{ t('framework.summary.graph.edges', '边') }} {{ graphStats.edge_count }}
        </div>
        <div class="st-ds" v-else>
          {{ t('framework.summary.graph.notGenerated', '未生成图模型') }}
        </div>
      </div>
    </template>
    ...
    <PlaceholderBanner
      ...
      type="failure"
      :title="compilation.compileError ?? t('framework.summary.failure.title', '编译请求失败')"
      :description="t('framework.summary.failure.description', '请检查源代码后重试')"
    />
  </div>
</template>
