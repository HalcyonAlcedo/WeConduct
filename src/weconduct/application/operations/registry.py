...
from .models import (
    ...
    ExecutionStartInput,
    DebugAbortInput,
    DebugNetworkQueryInput,
    DebugNetworkTraceInput,
    ...
    OAuthAuthorizationBeginInput,
    OAuthDeviceBeginInput,
    OAuthFlowInput,
    OAuthFlowSubmitInput,
    ProjectCreateInput,
    ProjectOpenInput,
    ProjectSaveInput,
...
class OperationRegistry:
    ...
    def build_stable_public(cls) -> "OperationRegistry":
        ...
        descriptors = {
            ...
            "debug.network.summary": descriptor("debug.network.summary", "session_id", "source", "summary", input_model=DebugNetworkQueryInput),
            "debug.network.list": descriptor("debug.network.list", "session_id", "source", "summary", "traces", input_model=DebugNetworkQueryInput),
            "debug.network.get": descriptor("debug.network.get", "session_id", "source", "trace", input_model=DebugNetworkTraceInput),
            ...
            "graph.compile": descriptor("graph.compile", "status", "request", "outcome", "view", "diagnostics", input_model=GraphCompileInput, side_effect_level=SideEffectLevel.WRITE, idempotency_capability=IdempotencyCapability.SUPPORTED),
            ...
            "oauth.authorization.begin": descriptor(
                "oauth.authorization.begin",
                "flow_id", "flow_type", "status", "scope_id", "request_id", "pending_input",
                "created_at", "error_code", "authorization_url", "redirect_uri",
                input_model=OAuthAuthorizationBeginInput,
                side_effect_level=SideEffectLevel.EXECUTE,
            ),
            "oauth.device.begin": descriptor(
                "oauth.device.begin",
                "flow_id", "flow_type", "status", "scope_id", "request_id", "pending_input",
                "created_at", "error_code", "verification_uri", "user_code", "expires_at", "interval",
                input_model=OAuthDeviceBeginInput,
                side_effect_level=SideEffectLevel.EXECUTE,
            ),
            "oauth.flow.get": descriptor(
                "oauth.flow.get",
                "flow_id", "flow_type", "status", "scope_id", "request_id", "pending_input",
                "created_at", "error_code", "authorization_url", "redirect_uri",
                "verification_uri", "user_code", "expires_at", "interval",
                "token_type", "token_expires_at",
                input_model=OAuthFlowInput,
            ),
            "oauth.flow.submit": descriptor(
                "oauth.flow.submit",
                "flow_id", "flow_type", "status", "scope_id", "request_id", "pending_input",
                "created_at", "error_code", "authorization_url", "redirect_uri",
                "verification_uri", "user_code", "expires_at", "interval",
                "token_type", "token_expires_at",
                input_model=OAuthFlowSubmitInput,
                side_effect_level=SideEffectLevel.EXECUTE,
            ),
            "oauth.flow.cancel": descriptor(
                "oauth.flow.cancel",
                "flow_id", "flow_type", "status", "scope_id", "request_id", "pending_input",
                "created_at", "error_code", "authorization_url", "redirect_uri",
                "verification_uri", "user_code", "expires_at", "interval",
