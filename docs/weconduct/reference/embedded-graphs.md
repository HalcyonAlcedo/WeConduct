...
- `root_metadata.graph_compatibility.built_with_app_version` 当前固定要求 `0.9.1`。
- `root_metadata.graph_compatibility.graph_data_version` 仍为 `0.9.0`；0.9.1 未引入图数据格式迁移。
- 教学图目前只支持 `control` 和 `data` 两类边；`observe` 边会被验证器拒绝。
- 节点需要真实 `position`，节点端口必须与 WeConduct 0.9.1 组件 manifest 契约一致。
