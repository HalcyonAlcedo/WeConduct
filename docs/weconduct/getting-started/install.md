...
2. 如果 ZIP 是浏览器下载的，先右键 ZIP，打开 **属性**，勾选 **解除锁定**，再解压。
3. 不要直接在压缩包预览窗口中运行程序。
4. 启动解压目录中的 `WeConduct.exe`。
5. Windows 首次提示来源确认时，核对发布来源后再继续。

浏览器下载的 ZIP 可能把“来自网络”安全标记传给解压后的 EXE 和 DLL，导致内部运行时组件在首次启动时被系统拦截。如果已经解压且启动失败，在 PowerShell 7 中进入程序目录的上一级，再执行：

```powershell
Get-ChildItem '.\WeConduct' -Recurse -File | Unblock-File
```

目录名不是 `WeConduct` 时，替换为实际目录。只对确认来自官方 Release 的文件执行解除锁定；不希望处理此问题时，优先使用安装包。
