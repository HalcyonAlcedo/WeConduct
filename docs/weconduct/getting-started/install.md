...
前往 [WeConduct Releases](https://github.com/HalcyonAlcedo/WeConduct/releases/tag/0.9.0) 下载页面，你会看到两个文件：

- **`WeConduct-0.9.0-portable.zip`**：便携版，解压后直接运行，适合想快速试用、随身携带或多版本并存的场景。
- **`WeConduct-0.9.0-setup.exe`**：安装版，标准的 Windows 安装程序，会帮你创建快捷方式和卸载入口。

> 如果下载页面显示的文件名和上面不一样，请以页面上的实际文件名为准。

## 便携版怎么用

1. 把 ZIP 文件完整解压到一个有写入权限的目录，比如 `D:\Apps\WeConduct-0.9.0`。
...
1. 双击运行 `WeConduct-0.9.0-setup.exe`。
