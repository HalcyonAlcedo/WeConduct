...
开启 `allow_file_access`，并确认路径位于项目目录、工作区目录、状态目录、用户 `Downloads` 或 `custom_roots`。0.9.1 不会自动允许用户 `Documents`。
...
0.9.1 的外部绑定值会解析为绝对路径，绑定目标只支持 `initial_variable`。输入真实路径字符串后重新检查运行就绪状态。
