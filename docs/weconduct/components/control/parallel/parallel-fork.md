...
该节点属于“并行控制”。实现类型为 `core_atomic`，运行展开角色为 `control:parallel_fork`。
...
输入：`in`。输出：`branch:left`、`branch:right`。副作用：改变后续控制路径。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/control/control-parallel-fork.json" title="并行分叉配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`branches`；端口不存在或关系层不匹配；输入类型与参数要求不一致；控制出口未连接、表达式无效或循环/重试边界配置不合法。诊断应保留节点 ID、资源键和原始错误信息。
