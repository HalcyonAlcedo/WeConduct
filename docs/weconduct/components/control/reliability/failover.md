...
资源键：`control.failover`　|　英文名：Failover
## 功能说明
...
输入端口：`in`。输出端口：`primary`、`fallback:backup`、`failed`。对外影响：改变后续执行路径。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/control/control-failover.json" title="故障切换配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`fallback_expression`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；控制出口未连接、表达式无效或循环/重试边界配置不合法。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
