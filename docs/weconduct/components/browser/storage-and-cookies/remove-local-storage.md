...
product: weconduct
version: 0.8.1
doc_id: component:browser.remove_local_storage
---
...
从当前浏览器上下文删除 localStorage 项。

该节点属于“存储与 Cookie”。实现类型为 `core_atomic`，运行展开角色为 `action:remove_local_storage`。
...
输入：`in`、`in:key`。输出：`out`。副作用：读取、修改或清除浏览器持久状态；写操作会影响后续页面请求。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-remove-local-storage.json" title="删除本地存储配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
