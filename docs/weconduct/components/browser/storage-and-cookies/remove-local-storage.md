...
资源键：`browser.remove_local_storage`　|　英文名：Remove Local Storage
## 功能说明
...
输入端口：`in`、`in:key`。输出端口：`out`。对外影响：读取、修改或清除浏览器持久状态；写操作会影响后续页面请求。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-remove-local-storage.json" title="删除本地存储配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
