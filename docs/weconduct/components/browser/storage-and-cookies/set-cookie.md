...
product: weconduct
version: 0.8.1
doc_id: component:browser.set_cookie
---
...
该节点属于“存储与 Cookie”。实现类型为 `core_atomic`，运行展开角色为 `action:set_cookie`。
...
| `http_only` | `boolean` | 否 | `false` | `default` |
| `secure` | `boolean` | 否 | `false` | `default` |
| `same_site` | `string` | 否 | `"Lax"` | `select` |
| `expires` | `any` | 否 | `null` | `default` |

## 输入、输出与副作用

输入：`in`。输出：`out`。副作用：读取、修改或清除浏览器持久状态；写操作会影响后续页面请求。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-set-cookie.json" title="写入 Cookie配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`name`、`value`、`url`、`domain`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足。诊断应保留节点 ID、资源键和原始错误信息。
