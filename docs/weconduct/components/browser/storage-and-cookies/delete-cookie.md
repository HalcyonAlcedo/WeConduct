...
该节点属于“存储与 Cookie”。实现类型为 `core_atomic`，运行展开角色为 `action:delete_cookie`。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-delete-cookie.json" title="删除 Cookie配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`name`、`url`、`domain`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足。诊断应保留节点 ID、资源键和原始错误信息。
