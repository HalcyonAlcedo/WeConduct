...
该节点属于“存储与 Cookie”。实现类型为 `core_atomic`，运行展开角色为 `action:clear_local_storage`。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-clear-local-storage.json" title="清空本地存储配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
