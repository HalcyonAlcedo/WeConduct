...
该节点属于“存储与 Cookie”。实现类型为 `core_atomic`，运行展开角色为 `action:set_session_storage`。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-set-session-storage.json" title="写入会话存储配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
