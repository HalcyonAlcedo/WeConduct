...
product: weconduct
version: 0.9.1
doc_id: component:browser.set_input_files
---

# 设置上传文件

资源键：`browser.set_input_files`　|　英文名：Set Input Files
## 功能说明
...
输入端口：`in`、`in:selector`、`in:path`。输出端口：`out`。对外影响：可能改变页面状态、浏览器上下文、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-set-input-files.json" title="设置上传文件配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
