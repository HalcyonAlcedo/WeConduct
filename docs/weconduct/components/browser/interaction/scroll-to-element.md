...
product: weconduct
version: 0.8.1
doc_id: component:browser.scroll_to_element
---
...
该节点属于“页面交互”。实现类型为 `core_atomic`，运行展开角色为 `action:scroll_to_element`。
...
| `block` | `string` | 否 | `"center"` | `default` |
| `inline` | `string` | 否 | `"nearest"` | `default` |
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-scroll-to-element.json" title="滚动到元素配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`selector`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足。诊断应保留节点 ID、资源键和原始错误信息。
