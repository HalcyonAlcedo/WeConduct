...
该节点属于“页面交互”。实现类型为 `core_atomic`，运行展开角色为 `action:select_option`。
...
输入：`in`、`in:selector`、`in:value`。输出：`out`。副作用：可能读取或改变页面、浏览器状态、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-select-option.json" title="选择选项配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
