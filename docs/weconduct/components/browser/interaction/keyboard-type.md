...
product: weconduct
version: 0.9.1
doc_id: component:browser.keyboard_type
---

# 键盘输入

资源键：`browser.keyboard_type`　|　英文名：Keyboard Type
## 功能说明
...
| `delay_ms` | `integer` | 否 | `0` | `default` |

## 输入、输出与副作用

输入端口：`in`、`in:text`。输出端口：`out`。对外影响：可能改变页面状态、浏览器上下文、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-keyboard-type.json" title="键盘输入配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
