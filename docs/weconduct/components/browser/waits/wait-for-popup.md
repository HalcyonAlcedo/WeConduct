...
product: weconduct
version: 0.8.1
doc_id: component:browser.wait_for_popup
---

# 等待弹窗

资源键：`browser.wait_for_popup`  
英文名：Wait For Popup
...
该节点属于“等待条件”。实现类型为 `core_atomic`，运行展开角色为 `action:wait_for_popup`。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-wait-for-popup.json" title="等待弹窗配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`variable_name`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足。诊断应保留节点 ID、资源键和原始错误信息。
