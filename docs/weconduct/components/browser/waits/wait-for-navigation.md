...
该节点属于“等待条件”。实现类型为 `core_atomic`，运行展开角色为 `action:wait_for_navigation`。
...
输入：`in`、`in:url_pattern`。输出：`out`。副作用：可能读取或改变页面、浏览器状态、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-wait-for-navigation.json" title="等待导航配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`url_pattern`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足。诊断应保留节点 ID、资源键和原始错误信息。
