...
product: weconduct
version: 0.8.1
doc_id: component:browser.wait_for_navigation
---

# 等待导航

资源键：`browser.wait_for_navigation`　|　英文名：Wait For Navigation
## 功能说明
...
| `in:url_pattern` | input | `data` | `in.url_pattern` |
...
| `timeout` | `integer` | 否 | `15000` | `default` |

## 输入、输出与副作用

输入端口：`in`、`in:url_pattern`。输出端口：`out`。对外影响：可能改变页面状态、浏览器上下文、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-wait-for-navigation.json" title="等待导航配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`url_pattern`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；运行环境、资源路径或安全权限未正确配置。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
