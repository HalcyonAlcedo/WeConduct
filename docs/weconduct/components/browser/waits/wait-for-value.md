...
资源键：`browser.wait_for_value`　|　英文名：Wait For Value
## 功能说明
...
输入端口：`in`、`in:selector`、`in:value`。输出端口：`out`、`out:value`。对外影响：可能改变页面状态、浏览器上下文、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-wait-for-value.json" title="等待值配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
