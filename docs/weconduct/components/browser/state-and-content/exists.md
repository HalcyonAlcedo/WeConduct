...
该节点属于“页面状态与内容”。实现类型为 `core_atomic`，运行展开角色为 `action:exists`。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-exists.json" title="元素存在配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
