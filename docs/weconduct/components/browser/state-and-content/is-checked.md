...
product: weconduct
version: 0.9.0
doc_id: component:browser.is_checked
---

# 元素已勾选

资源键：`browser.is_checked`　|　英文名：Is Checked
## 功能说明
...
输入端口：`in`、`in:selector`。输出端口：`out`、`out:value`。对外影响：可能改变页面状态、浏览器上下文、网络记录或本地文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-is-checked.json" title="元素已勾选配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
节点执行成功后，状态为 `succeeded`。你可以从 `out:value` 端口或节点输出字段获取结果。
