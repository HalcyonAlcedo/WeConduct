...
该节点属于“脚本与提取”。实现类型为 `core_atomic`，运行展开角色为 `action:run_js`。
...
输入：`in`、`in:script`。输出：`out`、`out:value`。副作用：脚本可读取或修改页面 DOM 和页面全局状态。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/browser/browser-run-js.json" title="运行 JS配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`script`、`variable_name`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足；脚本语法错误、返回值不可序列化或 JavaScript 权限未开启。诊断应保留节点 ID、资源键和原始错误信息。
