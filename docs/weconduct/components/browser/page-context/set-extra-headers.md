...
product: weconduct
version: 0.9.1
doc_id: component:browser.set_extra_headers
---

# 设置额外请求头

资源键：`browser.set_extra_headers`　|　英文名：Set Extra Headers
## 功能说明

为后续浏览器请求配置额外 HTTP 请求头。
...
<weconduct-graph src="../../../../assets/graphs/components/browser/browser-set-extra-headers.json" title="设置额外请求头配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`headers`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；运行环境、资源路径或安全权限未正确配置。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
