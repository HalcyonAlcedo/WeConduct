...
该节点属于“兼容与内部节点”。实现类型为 `core_atomic`，运行展开角色为 `action:call_subgraph`。
...
<weconduct-graph src="../../../../assets/graphs/components/internal/graph-call-subgraph.json" title="调用子图配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`subgraph_id`；端口不存在或关系层不匹配；输入类型与参数要求不一致；子图不存在、schema 不匹配或输入输出映射引用无效。诊断应保留节点 ID、资源键和原始错误信息。
