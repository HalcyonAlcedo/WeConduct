...
资源键：`call_blueprint`　|　英文名：Call Blueprint
## 功能说明
...
<weconduct-graph src="../../../../assets/graphs/components/internal/call-blueprint.json" title="调用蓝图配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`blueprint_id`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；子图不存在、schema 不匹配或输入输出映射引用无效。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
