...
product: weconduct
version: 0.9.0
doc_id: component:data.set_variables_batch
---

# 批量写入变量

资源键：`data.set_variables_batch`　|　英文名：Set Variables Batch
## 功能说明
...
输入端口：`in`。输出端口：`out`。对外影响：按 `variable_name` 或节点语义读取或更新运行时变量。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-set-variables-batch.json" title="批量写入变量配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；变量不存在、值类型不兼容或表达式/正则格式无效。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
