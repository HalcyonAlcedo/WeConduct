...
资源键：`data.set_variable`　|　英文名：Set Variable
## 功能说明
...
输入端口：`in`、`in:value`。输出端口：`out`、`out:value`。对外影响：按 `variable_name` 或节点语义读取或更新运行时变量。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-set-variable.json" title="写入变量配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
