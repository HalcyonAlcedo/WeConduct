...
资源键：`data.get_text`　|　英文名：Get Text
## 功能说明
...
<weconduct-graph src="../../../../assets/graphs/components/data/data-get-text.json" title="读取文本配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`selector`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；变量不存在、值类型不兼容或表达式/正则格式无效。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
