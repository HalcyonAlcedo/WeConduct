...
product: weconduct
version: 0.8.1
doc_id: component:data.get_element_count
---
...
该节点属于“页面取值”。实现类型为 `core_atomic`，运行展开角色为 `action:get_element_count`。
...
<weconduct-graph src="../../../../assets/graphs/components/data/data-get-element-count.json" title="读取元素数量配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`selector`、`variable_name`；端口不存在或关系层不匹配；输入类型与参数要求不一致；变量不存在、值类型不兼容或表达式/正则格式无效。诊断应保留节点 ID、资源键和原始错误信息。
