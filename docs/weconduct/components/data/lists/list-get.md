...
资源键：`data.list_get`　|　英文名：List Get
## 功能说明
...
输入端口：`in`、`in:index`。输出端口：`out`、`out:value`。对外影响：读取原列表并输出查询结果。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-get.json" title="列表读取配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
