...
该节点属于“列表处理”。实现类型为 `builtin_custom_component`，运行展开角色为 `action:list_extend`。
...
输入：`in`、`in:items`。输出：`out`、`out:value`。副作用：直接更新原列表变量。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-extend.json" title="列表扩展配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
