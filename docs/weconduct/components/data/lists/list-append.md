...
product: weconduct
version: 0.9.1
doc_id: component:data.list_append
---

# 列表追加

资源键：`data.list_append`　|　英文名：List Append
## 功能说明
...
输入端口：`in`、`in:value`。输出端口：`out`、`out:value`。对外影响：直接更新原列表变量。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-append.json" title="列表追加配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
