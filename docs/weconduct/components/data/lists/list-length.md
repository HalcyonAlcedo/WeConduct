...
product: weconduct
version: 0.8.1
doc_id: component:data.list_length
---
...
该节点属于“列表处理”。实现类型为 `builtin_custom_component`，运行展开角色为 `action:list_length`。
...
输入：`in`。输出：`out`、`out:value`。副作用：读取原列表并输出查询结果。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-length.json" title="列表长度配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
