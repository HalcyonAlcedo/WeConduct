...
product: weconduct
version: 0.8.1
doc_id: component:data.list_slice
---
...
该节点属于“列表处理”。实现类型为 `builtin_custom_component`，运行展开角色为 `action:list_slice`。
...
| `in:start` | input | `data` | `in.start` |
| `in:end` | input | `data` | `in.end` |
...
| `start` | `integer` | 否 | `0` | `default` |
| `end` | `integer` | 否 | `null` | `default` |
| `output_variable_name` | `string` | 是 | `""` | `default` |

## 输入、输出与副作用

输入：`in`、`in:start`、`in:end`。输出：`out`、`out:value`。副作用：读取原列表并输出查询结果。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-slice.json" title="列表切片配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`variable_name`、`output_variable_name`；端口不存在或关系层不匹配；输入类型与参数要求不一致；目标变量不是列表、索引越界或待查找值不存在。诊断应保留节点 ID、资源键和原始错误信息。
