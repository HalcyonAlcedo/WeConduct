...
product: weconduct
version: 0.8.1
doc_id: component:data.list_insert
---

# 列表插入

资源键：`data.list_insert`　|　英文名：List Insert
## 功能说明
...
输入端口：`in`、`in:index`、`in:value`。输出端口：`out`、`out:value`。对外影响：直接更新原列表变量。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-insert.json" title="列表插入配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`variable_name`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；目标变量不是列表、索引越界或待查找值不存在。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
