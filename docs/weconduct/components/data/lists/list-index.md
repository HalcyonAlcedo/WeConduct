...
该节点属于“列表处理”。实现类型为 `builtin_custom_component`，运行展开角色为 `action:list_index`。
...
输入：`in`、`in:value`。输出：`out`、`out:value`。副作用：读取原列表并输出查询结果。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/data/data-list-index.json" title="列表索引配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
