...
该节点属于“流程与组件边界”。实现类型为 `core_atomic`，运行展开角色为 `component:input`。
...
输入：无显式输入端口。输出：`out`。副作用：在父图与子图之间映射变量和控制上下文。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/flow/component-input.json" title="组件输入配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
