...
product: weconduct
version: 0.9.0
doc_id: component:excel.read_row
---

# 读取 Excel 行

资源键：`excel.read_row`　|　英文名：Read Excel Row
## 功能说明
...
| `row_index` | `integer` | 否 | `1` | `default` |
| `variable_name` | `string` | 是 | `""` | `default` |

## 输入、输出与副作用

输入端口：`in`。输出端口：`out`、`out:row`。对外影响：读取工作簿，写入类节点还会更新并保存目标文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/excel/excel-read-row.json" title="读取 Excel 行配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
