...
product: weconduct
version: 0.9.1
doc_id: component:excel.write_cell
---

# 写入 Excel 单元格

资源键：`excel.write_cell`　|　英文名：Write Excel Cell
## 功能说明
...
输入端口：`in`、`in:value`。输出端口：`out`。对外影响：读取工作簿，写入类节点还会更新并保存目标文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/excel/excel-write-cell.json" title="写入 Excel 单元格配置示例">如果图示加载失败，可以查看下方的示例配置。</weconduct-graph>
...
缺少必填参数：`path`、`cell`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；运行环境、资源路径或安全权限未正确配置；工作簿或工作表不存在、单元格/行参数无效或文件被占用。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。
