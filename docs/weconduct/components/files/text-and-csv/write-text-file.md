...
该节点属于“文本与 CSV 文件”。实现类型为 `未声明`，运行展开角色为 `action:write_text_file`。
...
| `content` | `string` | 否 | `""` | `textarea` |

## 输入、输出与副作用

输入：`in`、`in:content`。输出：`out`。副作用：可能读取或写入文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/file/file-write-text-file.json" title="写入文本文件配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
...
缺少必填参数：`path`；端口不存在或关系层不匹配；输入类型与参数要求不一致；运行环境、资源路径或安全权限未满足；路径越界、编码不匹配、CSV 行列越界或文件不可访问。诊断应保留节点 ID、资源键和原始错误信息。
