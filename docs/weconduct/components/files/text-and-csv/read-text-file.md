...
该节点属于“文本与 CSV 文件”。实现类型为 `未声明`，运行展开角色为 `action:read_text_file`。
...
输入：`in`。输出：`out`、`out:text`。副作用：可能读取或写入文件。

## 使用示例

<weconduct-graph src="../../../../assets/graphs/components/file/file-read-text-file.json" title="读取文本文件配置示例">图示加载失败时，可阅读下方配置。</weconduct-graph>
