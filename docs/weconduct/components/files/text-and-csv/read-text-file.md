...
输入端口：`in`。输出端口：`out`、`out:text`。对外影响：读取目标文本文件，不修改文件内容。
...
返回文件路径、编码和 `content`；当前节点不使用 CSV 行列参数，也不会执行写入。

## 常见问题

缺少必填参数：`path`；端口名称写错或关系层不匹配；输入值的类型与参数要求不一致；运行环境、资源路径或安全权限未正确配置；路径越界、文件不存在、编码不匹配或文件不可访问。排查问题时，建议记录下节点 ID、资源键和原始错误信息，方便定位原因。

## 注意事项

读取结果会进入运行时输出；需要写入变量时应使用明确的数据输出或后续变量节点；示例图只展示了节点的配置结构；要构成完整流程，还需要添加入口节点、控制边和业务输入。
